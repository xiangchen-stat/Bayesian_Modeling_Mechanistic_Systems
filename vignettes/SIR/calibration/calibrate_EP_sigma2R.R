...

{
  ...
  Yall.file.name <- file.path(sprintf("Y_train_all_Zparms%d.csv", zparm_ix))
...

{
  ...
  calib.out.dir <- file.path(top.dir, 
  			 sprintf("vignettes/calibration/LV_Zparms%d_zreal_u%dv%d_AR%d_%dT_%dsample_calib_%s", 
  ...
  em.in.dir <- file.path(top.dir,
			 sprintf("vignettes/emulation/LV_train_Zparms%d_u%dv%d_AR%d_T%d_%s",
  ...
  # filter out the "burn-in" just in case.
  #etas <- etas[(L_burn+1):nrow(etas),]
