...
for (t in 1:nT) {
  ...
  for (s in 1:nsam) {
    ...
    #                                U = V_ls, V = res_ffbs$Sigma[s] * R, log = TRUE)
    loglik_train[s, t] <- mniw::dMNorm(X = Y[[t]], Lambda = F_ls[[t]] %*% res_ffbs[[t]][,,s],
                                       SigmaR = V_ls, SigmaC = res_ffbs$Sigma[s] * R, log=TRUE)
  }
}
# save(loglik_train, file = here("data", "loglik_train_MNIG_200.RData"))
...
res_pre <- FFBS_predict_MC(nsam = nsam, Y = Y, res_ffbs = res_ffbs,
