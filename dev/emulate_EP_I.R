...
setwd("D:/Documents/UCLA/0-Administrative/GSR/Bayesian-Modeling-Mechanistic-Systems/dev")
source("init_lib.r")
seed <- 1234
set.seed(seed)
...
F_ls_test <- list()
F_ls_train <- list()
F_ls_train_full <- gen_F_ls_AR1(N = n_train, nT = nT, Y = Y_full)
F_ls_test_full <- gen_F_ls_AR1(N = n_test, nT = nT, Y = Y_test_full)
for (i in 1:nT) {
  temp_F <- F_ls_train_full[[i]]
  for (j in 1:n_b) {
    F_ls_train[[(i-1)*n_b + j]] <- temp_F[ind[j,2]:ind[j,3], ind[j,4]:ind[j,5]]
  }
  # 
  # if (i == nT) {
  #   save(F_ls_train, file = file.path(paste(path_data, "/F_ls_train.RData", sep = "")))
  # }
}

for (i in 1:nT) {
  temp_F <- F_ls_test_full[[i]]
  for (j in 1:n_b_test) {
    F_ls_test[[(i-1)*n_b_test + j]] <- temp_F[ind_test[j,2]:ind_test[j,3], ind_test[j,4]:ind_test[j,5]]
  }
  # 
  # if (i == nT) {
  #   save(F_ls_test, file = file.path(paste(path_data, "/F_ls_test.RData", sep = "")))
  # }
}
print("using AR1")

# F_ls_train_full <- gen_F_ls_AR2(N = n_train, nT = nT, Y = Y)
# F_ls_test_full <- gen_F_ls_AR2(N = n_test, nT = nT, Y = Y_test)
# print("using AR2")

# set Y
# if(ind_old_data == TRUE){
#   load(file.path(paste(path_data, "/Y.RData", sep = "")))
#   load(file.path(paste(path_data, "/V_para.RData", sep = "")))
# } else{
  for (i in 1:nT) {
    # if(i %% 10 == 0){
    #   print(paste("set Y and V", i, "/", nT))
    # }
    temp_Y <- Y_full[[i]]
    # temp_F <- F_ls_train_full[[i]]
    for (j in 1:n_b) {
      Y[[(i-1)*n_b + j]] <- temp_Y[ind[j,2]:ind[j,3], ind[j,4]:ind[j,5]]
      # F_ls_train[[(i-1)*n_b + j]] <- temp_F[ind[i,2]:ind[i,3],]
      # V_para[[(i-1)*n_b + j]] <- V_para_full[ind[j,2]:ind[j,3], ind[j,2]:ind[j,3]]
    }
    
    # if (i == nT) {
    #   save(Y, file = file.path(paste(path_data, "/Y.RData", sep = "")))
    #   # save(F_ls_train, file = file.path(paste(path_data, "/F_ls_train.RData", sep = "")))
    #   # save(V_para, file = file.path(paste(path_data, "/V_para.RData", sep = "")))
    # }
  }
# }
...
time_start <- Sys.time()
time_start
para_ffbs <- FFBS_I(Y = Y, F_ls = F_ls, G_ls = G_ls,
                      W_ls = W_ls, V_ls = V_ls,
                      m0 = m0, M0 = M0,
                      nT = nT, delta = 1.0)
time_end <- Sys.time()
...
time_start <- Sys.time()
time_start
res_ffbs <- FFBS_sampling_I(nsam = nsam, para_ffbs = para_ffbs, 
                               F_ls = F_ls, G_ls = G_ls,
                               nT = nT, delta = 1)
time_end <- Sys.time()
time_end
...
res_pre_epi <- FFBS_predict_MC(nsam = nsam, Y = Y, res_ffbs = res_ffbs,
                                    input = input, input_new = input_new,
                                    F_ls = F_ls, F_new_ls = F_new_ls,
                                    nT = nT, delta = 1.0)
res_pre_exact_epi <- FFBS_predict_exact(Y = Y, para_ffbs = para_ffbs,
                                    input = input, input_new = input_new,
                                    F_ls = F_ls, F_new_ls = F_new_ls,
                                    nT = nT, delta = 1.0)
...
res_pre_exact <- list()
res_pre_exact <- transform_from_episode(res_pre_exact_epi$post_mean, nT_ori = nT_ori, nT = nT)
...
dat <- res_pre_exact$post_mean # this is exact
