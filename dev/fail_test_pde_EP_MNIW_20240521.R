...
V_para_full <- gen_expsq_kernel(loc = pde_para_train, phi = phi_para, sigma2 = 1.1, tau2 = 10^(-4)) # exponential kernel
...
if(ind_old_data == TRUE){
  ...
  for (i in 1:nT) {
    ...
    for (j in 1:n_b) {
      Y[[(i-1)*n_b + j]] <- temp_Y[ind[i,2]:ind[i,3], ind[i,4]:ind[i,5]]
      # F_ls_train[[(i-1)*n_b + j]] <- temp_F[ind[i,2]:ind[i,3],]
      V_para[[(i-1)*n_b + j]] <- V_para_full[ind[i,2]:ind[i,3], ind[i,2]:ind[i,3]]
    }
