...
dMTig <- function(Y, m, M, nu, d, R, log=TRUE) {
  ...
  out <- lgamma(p*S/2 + nu) - lgamma(nu) - p*S/2 * log(2 * pi * d) - 
    ...
    (p*S/2 + nu) * LaplacesDemon::logdet(temp)
...
rowSums(lppd_ana) / 10^5
# lppd_ana[, 45:55]


## lppd MC mean----
{
  SigmaC <- apply(res_ffbs_mniw$Sigma, c(1,2), mean)
  ## MNIW
  time_start <- Sys.time()
  ...
  for (t in 1:nT) {
    ...
    loglik_mcmean_mniw[1, t] <- mniw::dMNorm(X = Y[[t]], Lambda = F_ls[[t]] %*% apply(res_ffbs_mniw[[t]], c(1,2), mean),
                                             SigmaR = V_ls, SigmaC = SigmaC, log = TRUE)
  }
  time_end <- Sys.time()
  print(time_end - time_start)
...
rowSums(loglik_mcmean_mniw) / 10^5
# rowSums(loglik_mcmean_mnig) / 10^5
# rowSums(loglik_mcmean_i) / 10^5
